...
@app.route('/teste/<canal>')
def rota_teste(canal):
    out = []
    canal = canal.strip('/').lower()
    out.append(f"=== TESTE CANAL: {canal} ===")
    ...
    for var in variacoes:
        for base in EMBEDS_DOMINIOS:
            url = f"{base}/{var}"
            try:
                r = sess.get(url, headers={"User-Agent": USER_AGENT}, timeout=6, verify=False)
                links = extrair_links_playlist(r.text) if r.status_code == 200 else []
                out.append(f"{url} -> HTTP {r.status_code} | {len(r.text)} bytes | {len(links)} links m3u8")
            except Exception as e:
                out.append(f"{url} -> ERRO: {e}")

    out.append("\n--- 2) TESTANDO BASE_SELENIUM ---")
    for var in variacoes:
        url = f"{BASE_SELENIUM}/{var}"
        try:
            r = SESSAO.get(url, timeout=8, verify=False)
            out.append(f"{url} -> HTTP {r.status_code} | {len(r.text)} bytes")
            if r.status_code == 200:
                m = re.search(r'["\'](https?://[^"\']+\.m3u8[^"\']*)["\']', r.text)
                out.append(f"  m3u8: {m.group(1) if m else 'NAO ENCONTRADO'}")
        except Exception as e:
            out.append(f"{url} -> ERRO: {e}")

    out.append("\n--- 3) PERFIS LOCAIS ---")
    out.append(f"Perfis: {listar_perfis()}")

    out.append("\n--- 4) RESULTADO FINAL ---")
    resp, url_a, cfg, tunel = processar_embed(canal)
    if resp:
        out.append(f"processar_embed: OK -> {url_a}")
    else:
        out.append("processar_embed: FALHOU")
    resp, url_a, cfg, tunel = processar_embed_especial(canal)
    if resp:
        out.append(f"processar_embed_especial: OK -> {url_a}")
    else:
        out.append("processar_embed_especial: FALHOU")
    resp, url_a, cfg, tunel = processar_bruto(canal)
    if resp:
        out.append(f"processar_bruto: OK -> {url_a}")
    else:
        out.append("processar_bruto: FALHOU")
...
if __name__ == '__main__':
    ...
    print(f"Teste:   {URL_PUBLICA}/teste/tnt")
